...
struct CreateAccountAction: CBOREncodable {
    func encode() -> [UInt8] {
        let cborWrapper : CBOR = [
            "label": CBOR.utf8String(self.label)
//            ,"unknownFields": CBOR(nilLiteral: self.unknownFields)
        ]
...
struct SendPrescriptAction: CBOREncodable {
        func encode() -> [UInt8] {
            let cborWrapper : CBOR = [
                "index": CBOR.utf8String(self.index),
                "hash": CBOR.utf8String(self.hash),
                "patPublicKey": CBOR.utf8String(self.patPublicKey),
                "docPublicKey": CBOR.utf8String(self.docPublicKey)
            ]
            return cborWrapper.encode()
        }
  // SwiftProtobuf.Message conformance is added in an extension below. See the
